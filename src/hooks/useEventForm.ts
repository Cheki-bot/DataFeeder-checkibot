...
type EventFormAction =
    ...
    | {
          ...
          payload: { index: number; data: EventFormData };
      }
    | { type: 'PREPARE_EDIT_EVENT'; payload: number }
    | { type: 'CLOSE_EVENT_MODAL' }
    | {
          type: 'UPDATE_EVENT_FIELD';
          payload: { field: keyof EventFormData; value: string | number };
      }
        ...
        case 'PREPARE_EDIT_EVENT': {
            ...
            const formData: EventFormData = {
                ...
                to_date: event.to_date
                    ? new Date(event.to_date).toISOString().split('T')[0]
                    : '',
...
export const useEventForm = (): UseEventFormReturn => {
    ...
    const updateEventField = useCallback(
        (field: keyof EventFormData, value: string | number) => {
        ...
        async (calendarId: string) => {
            ...
            try {
                ...
                const newEvent = {
                    ...
                    from_date: state.eventForm.from_date
                        ? new Date(state.eventForm.from_date)
                        : undefined,
                    to_date: state.eventForm.to_date
                        ? new Date(state.eventForm.to_date)
                        : undefined,
                ...
                const response = await updateCalendar(calendarId, {
                ...
                dispatch({
                    type: 'SET_ERROR',
                    payload: 'Error al guardar el evento',
                });
            } finally {
    ...
    const deleteEvent = useCallback(
        async (calendarId: string, index: number) => {
            if (!state.calendar) return;
            ...
            try {
                ...
                let updatedEvents = state.calendar.events.filter(
                    (_, i) => i !== index
                );
                ...
                dispatch({
                    type: 'SET_ERROR',
                    payload: 'Error al eliminar el evento',
                });
            }
        },
        [state.calendar]
